    ...
    // Fix: If FF is defined, BASE_PATH should be just NEW*DRIVE/FF (no extra drive/)
    // If FF is null, fallback to NEW*DRIVE/drive/
    const BASE_PATH = FF ? `NEW*DRIVE/${FF}` : "NEW*DRIVE/drive/";
    ...
    function renderFolders() {
      ...
      folders.forEach((folder) => {
        ...
        li.className =
          "cursor-pointer hover:bg-blue-100 px-4 py-3 flex items-center gap-3 select-none rounded";
    ...
    function customMarkdownRender(md) {
      ...
      function inlineReplacements(text) {
        ...
        text = text.replace(
          /`([^`\n]+)`/g,
          '<code class="bg-gray-100 rounded px-1 font-mono text-sm">$1</code>'
        );
        ...
        text = text.replace(
          /!\[([^\]]*)\]\(([^)]+)\)/g,
          '<img src="$2" alt="$1" class="max-w-full my-2 rounded shadow-md"/>'
        );
    ...
    async function openFolder(folder) {
      ...
      if (folder === null) {
        // Fix: When opening root folder, do not reload everything, just render from cached data
        renderFoldersInside(null);
        renderFiles(null);
        updateHash("", "");
        return;
    ...
    async function openFileFromHash() {
      ...
      if (
        folderKey !== null &&
        !folders.includes(folderKey) &&
        !Object.keys(foldersByFolder).some((k) => folderKey.startsWith(k))
      )
        return;
