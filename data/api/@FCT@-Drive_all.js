    ...
    // Get repo path from div#repo/${chemin}
    function getRepoPathFromDiv() {
      const divs = document.querySelectorAll("div[id^='repo/']");
      if (divs.length === 0) return "";
      // Take first div with id starting with repo/
      const div = divs[0];
      const id = div.id;
      // id format: repo/${chemin}
      if (!id.startsWith("repo/")) return "";
      return id.substring(5); // remove "repo/"
    }

    // Compose full base path for GitHub API calls
    function getBasePath() {
      const repoPath = getRepoPathFromDiv();
      if (!repoPath) return BASE_PATH_PREFIX;
      // Join with * as in original BASE_PATH_PREFIX
      return `${BASE_PATH_PREFIX}/${repoPath}`;
    }
    ...
    async function githubApi(path) {
      ...
      const url = `https://api.github.com/repos/${OWNER}/${REPO}/contents/${encodeURIComponent(fullPath)}?ref=${BRANCH}`;
    ...
    async function createFile(folder, filename) {
      ...
      const basePath = getBasePath();
      const path = folder === "Non trier" ? `${basePath}/${filename}` : `${basePath}/${folder}/${filename}`;
      try {
    ...
    async function loadRoot() {
      ...
      try {
        const basePath = getBasePath();
        const rootContent = await githubApi("");
        ...
        for (const folderName of rootFolders.map((f) => f.name)) {
          const folderContent = await githubApi(folderName);
    ...
    async function refreshFolderFiles(folder) {
      try {
        ...
        if (folder === "Non trier") {
          ...
          const folderContent = await githubApi(folder);
