   let BASE_PATH = null;
    ...
    async function loadRoot() {
      ...
      try {
        ...
        for (const folderName of rootFolders.map((f) => f.name)) {
          ...
          foldersByFolder[folderName] = folderContent
    ...
    async function reloadCurrentFolder() {
      if (currentFolder === null) {
        ...
        try {
          ...
          foldersByFolder[currentFolder] = folderContent
            .filter((item) => item.type === "dir")
            .map((d) => d.name);
    ...
    async function openFolder(folder) {
      ...
      if (folder === null) {
        ...
        try {
          ...
          foldersByFolder[folder] = folderContent
            .filter((item) => item.type === "dir")
            .map((d) => d.name);
        } catch (e) {
    ...
    // Disable save button and editing for client (read-only)
    btnSave.disabled = true;
    btnSave.classList.add("hidden");
